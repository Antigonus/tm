...
;;
  ...
  (defun inc-rightside-addresses-2 (tm)
  ...
  (defun-typed a ((tm ea2-active) instance &optional ➜)
    (destructuring-bind
      (&key
        (➜ok (be t))
        &allow-other-keys
...
;; solo-tm-decl-only
;;
  ;; see ea2-definitions for a◧ and d◧

  (defun dec-rightside-addresses-2 (tm)
    (bt:with-lock-held ((lock (locked-entanglements tm)))
      (c◧∀* (entanglements (locked-entanglements tm))
        (λ(es)
          (let(
  ...
  (defun-typed d ((tm ea2-active) &optional spill ➜)
    (destructuring-bind
      (&key
        (➜ok #'echo)
