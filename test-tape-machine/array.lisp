...
(defun test-tape-array-2 ()
  (let*(
    ...
    (let*(
           ...
           (v  (right-neighbor c2 {:➜ok (be 100) :➜right-bound (be 200)}))
           )
      (∧
        (= (r<cell> c0) 4)
        (w<cell> c0 41)
...
(defun test-tape-array-4 ()
  (let*(
    ...
    (let*(
      ...
      (∧
        (epd<tape> tp0 {:➜ok (be ∅) :➜right-bound (be t)})
        (epd<tape> tp1 {:➜ok (λ(c)(= (r<cell> c) 1)) :➜right-bound (be ∅)})
        (= (r<cell> c1) 5)
        (d<cell> c1 {:➜ok (λ(c)(= (r<cell> c) 6)) :➜right-bound (be ∅)})
        (d<cell> c1 {:➜ok (be ∅) :➜right-bound (be t)})
        (d.<cell> (left-bound tp3) {:➜ok (λ(c)(= (r<cell> c) 1)) :➜right-bound (be ∅)})
        (d.<cell> (left-bound tp3) {:➜ok (λ(c)(= (r<cell> c) 2)) :➜right-bound (be ∅)})
        (d.<cell> (left-bound tp3) {:➜ok (be ∅) :➜right-bound (be t)})
        (◧d.<tape> tp4 {:➜ok (λ(c)(= (r<cell> c) 17)) :➜right-bound (be ∅)})
        (◧d.<tape> tp4 {:➜ok (λ(c)(= (r<cell> c) 18)) :➜right-bound (be ∅)})
        (◧d.<tape> tp4 {:➜ok (λ(c)(= (r<cell> c) 19)) :➜right-bound (be ∅)})
